...
namespace Downloads.Plugins.SimpleInput.Scripts.AxisInputs
{
    ...
    public class AxisInputRotateGesture : MonoBehaviour, ISimpleInputDraggableMultiTouch
    {
            ...
            ISimpleInputDraggableMultiTouch activeListener)
        {
            ...
            float deltaAngle =
                (Mathf.Atan2(prevDeltaPosition.y, prevDeltaPosition.x) - Mathf.Atan2(deltaPosition.y, deltaPosition.x)) *
                MULTIPLIER;
            if (deltaAngle > 180f)
                deltaAngle -= 360f;
